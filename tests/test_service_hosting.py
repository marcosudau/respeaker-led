...
from src.service_hosting import (
    ActiveServiceInfo,
...
def test_take_over_existing_instance_requests_shutdown(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("src.service_hosting.LocalControllerClient", FakeClient)
    monkeypatch.setattr("src.service_hosting._pid_exists", lambda pid: True)
    monkeypatch.setattr("src.service_hosting._wait_for_pid_exit", lambda pid, timeout: True)
