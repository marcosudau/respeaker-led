...
class OverlayFocusEffect(BaseEffect):
    definition = EffectDefinition(
        id="overlay_focus",
        title="OverlayFocusEffect",
        description="Live smoke-test effect",
        parameter_schema={
            "color": EffectParamDefinition(name="color", type="color", default="0xFF66CC"),
        },
        defaults={"color": "0xFF66CC"},
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        raw_color = str(ctx.params.get("color", "0xFF66CC")).replace("#", "0x")
        base_color = int(raw_color, 16)
        leds = [None] * ctx.led_count
        leds[3 % ctx.led_count] = base_color
        leds[(3 + 1) % ctx.led_count] = base_color
        return leds
