...
class ListeningState(BaseEffect):
    definition = EffectDefinition(
        id='listening',
        title='Zuhören',
        description='Ruhiges cyanfarbenes Atmen zeigt ein aktives Mikrofon und die Bereitschaft für Sprache.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#32D7FF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=0.65, minimum=0.05, maximum=3.0, unit='multiplier'),
            'min_brightness': EffectParamDefinition(name='min_brightness', type='float', default=0.22, minimum=0.0, maximum=1.0, unit='ratio'),
        },
        defaults={
            'color': '#32D7FF',
            'brightness': 0.68,
        ...
        animated=True,
        directional=False,
        tags=('smartspeaker-set', 'state', 'listening', 'breathing'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        level = _breath(_elapsed(ctx), _float(params.get('speed'), 0.65), _clamp01(_float(params.get('min_brightness'), 0.22)))
        return [_scaled(params, 'color', 0x32D7FF, level)] * ctx.led_count
