...
class ThinkingState(BaseEffect):
    definition = EffectDefinition(
        id='thinking',
        title='Denken',
        description='Ein breiter, weich auslaufender lila Sweep läuft gleichmäßig um den Ring.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#A855F7'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=0.48, minimum=0.05, maximum=3.0, unit='multiplier'),
            'reverse': EffectParamDefinition(name='reverse', type='bool', default=False),
            'width': EffectParamDefinition(name='width', type='int', default=5, minimum=1, maximum=12),
            'falloff': EffectParamDefinition(name='falloff', type='float', default=0.55, minimum=0.1, maximum=0.95, unit='ratio'),
        },
        defaults={
            'color': '#A855F7',
            'brightness': 0.76,
        ...
        animated=True,
        directional=True,
        tags=('smartspeaker-set', 'state', 'thinking', 'sweep'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if ctx.led_count:
            ...
            center = int(math.floor(_elapsed(ctx) * _float(params.get('speed'), 0.48) * ctx.led_count * direction)) % ctx.led_count
            width = max(1, min(ctx.led_count, _int(params.get('width'), 5)))
            falloff = max(0.10, min(0.95, _float(params.get('falloff'), 0.55)))
            half = max(1.0, width / 2.0)
            for offset in range(-width, width + 1):
                distance = abs(offset) / half
                factor = math.exp(-distance * max(0.25, 1.0 / falloff))
                index = (center + offset * direction) % ctx.led_count
                current = frame[index] or 0
                candidate = _scaled(params, 'color', 0xA855F7, factor)
                frame[index] = candidate if candidate > current else current
        return frame
