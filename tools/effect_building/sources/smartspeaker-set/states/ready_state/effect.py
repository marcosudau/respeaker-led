...
class ReadyState(BaseEffect):
    definition = EffectDefinition(
        id='ready_state',
        title='Bereit / Idle',
        description='Sanftes, langsames Atmen als unaufdringlicher Bereitschaftszustand.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#35D06F'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=0.45, minimum=0.05, maximum=3.0, unit='multiplier'),
            'min_brightness': EffectParamDefinition(name='min_brightness', type='float', default=0.16, minimum=0.0, maximum=1.0, unit='ratio'),
        },
        defaults={
            'color': '#35D06F',
            'brightness': 0.55,
        ...
        animated=True,
        directional=False,
        tags=('smartspeaker-set', 'state', 'ready', 'breathing'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        level = _breath(_elapsed(ctx), _float(params.get('speed'), 0.45), _clamp01(_float(params.get('min_brightness'), 0.16)))
        color = _scaled(params, 'color', 0x35D06F, level)
