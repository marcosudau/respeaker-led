...
class TranscribeState(BaseEffect):
    definition = EffectDefinition(
        id='transcribe',
        title='Transkription',
        description='Ein fokussierter heller Kopf mit kurzem weichem Schweif rotiert zügig um den Ring.',
        ...
        parameter_schema={
            ...
            'speed': EffectParamDefinition(name='speed', type='float', default=1.35, minimum=0.05, maximum=3.0, unit='multiplier'),
            ...
            'falloff': EffectParamDefinition(name='falloff', type='float', default=0.62, minimum=0.15, maximum=0.95, unit='ratio'),
        },
        defaults={
            'color': '#2478FF',
            'brightness': 0.95,
        ...
        animated=True,
        directional=True,
        tags=('smartspeaker-set', 'state', 'transcribe', 'rotation'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if ctx.led_count:
            ...
            head = int(math.floor(_elapsed(ctx) * _float(params.get('speed'), 1.35) * ctx.led_count * direction)) % ctx.led_count
            length = max(1, min(ctx.led_count, _int(params.get('trail_length'), 4)))
            falloff = max(0.15, min(0.95, _float(params.get('falloff'), 0.62)))
            for offset in range(length):
                factor = falloff ** offset
                frame[(head - offset * direction) % ctx.led_count] = _scaled(params, 'color', 0x2478FF, factor)
        return frame
