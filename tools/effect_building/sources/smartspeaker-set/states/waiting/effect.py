...
class WaitingState(BaseEffect):
    definition = EffectDefinition(
        id='waiting',
        title='Wartend',
        description='Ein einzelner, langsam um den Ring laufender Lichtpunkt signalisiert Bereitschaft für ein Ereignis.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#3DDCFF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=0.28, minimum=0.05, maximum=3.0, unit='multiplier'),
        ...
        animated=True,
        directional=True,
        tags=('smartspeaker-set', 'state', 'waiting', 'rotation'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if ctx.led_count:
            head = int(math.floor(_elapsed(ctx) * _float(params.get('speed'), 0.28) * ctx.led_count * _direction(params))) % ctx.led_count
            frame[head] = _scaled(params, 'color', 0x3DDCFF)
        return frame
