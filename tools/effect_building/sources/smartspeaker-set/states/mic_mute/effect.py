...
class MicMuteState(BaseEffect):
    definition = EffectDefinition(
        id='mic_mute',
        title='Mikrofon stumm',
        description='Statisches rotes, vierfach symmetrisches X-artiges Ringmuster.',
        ...
        parameter_schema={
            ...
            'segment_length': EffectParamDefinition(name='segment_length', type='int', default=1, minimum=1, maximum=4),
        },
        defaults={
            'color': '#FF3038',
            'brightness': 0.9,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'state', 'mute'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if ctx.led_count:
            length = max(1, min(ctx.led_count, _int(params.get('segment_length'), 1)))
            color = _scaled(params, 'color', 0xFF3038)
            for fraction in (0.125, 0.375, 0.625, 0.875):
                center = round(fraction * ctx.led_count) % ctx.led_count
                start = center - (length // 2)
                _paint(frame, start, length, color)
        return frame
