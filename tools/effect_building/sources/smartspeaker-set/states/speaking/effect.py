...
class SpeakingState(BaseEffect):
    definition = EffectDefinition(
        id='speaking',
        title='Spricht',
        description='Mehrere symmetrische Segmente pulsieren rhythmisch mit leicht versetzten Phasen.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#36CFFF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=1.25, minimum=0.05, maximum=3.0, unit='multiplier'),
            'min_brightness': EffectParamDefinition(name='min_brightness', type='float', default=0.18, minimum=0.0, maximum=1.0, unit='ratio'),
            'segment_length': EffectParamDefinition(name='segment_length', type='int', default=2, minimum=1, maximum=6),
        },
        defaults={
            'color': '#36CFFF',
            'brightness': 0.9,
        ...
        animated=True,
        directional=False,
        tags=('smartspeaker-set', 'state', 'speaking', 'rhythmic'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if not ctx.led_count:
            return frame
        segment_length = max(1, min(ctx.led_count, _int(params.get('segment_length'), 2)))
        group_count = min(4, max(2, ctx.led_count // max(2, segment_length + 1)))
        minimum = _clamp01(_float(params.get('min_brightness'), 0.18))
        speed = max(0.05, _float(params.get('speed'), 1.25))
        for group in range(group_count):
            phase = (_elapsed(ctx) * speed + group / group_count) % 1.0
            wave = 0.5 - 0.5 * math.cos(2.0 * math.pi * phase)
            level = minimum + (1.0 - minimum) * wave
            start = round(group * ctx.led_count / group_count)
            _paint(frame, start, segment_length, _scaled(params, 'color', 0x36CFFF, level))
        return frame
