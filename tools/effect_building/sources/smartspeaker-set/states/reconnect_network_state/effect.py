...
class ReconnectNetworkState(BaseEffect):
    definition = EffectDefinition(
        ...
        title='Netzwerk-Reconnect',
        description='Alternierende helle und dunkle gelbe LEDs atmen gemeinsam, solange die Serververbindung fehlt.',
        definition_type=DefinitionType.STATE,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#FFD84D'),
            'secondary_color': EffectParamDefinition(name='secondary_color', type='color', default='#7A5A00'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'speed': EffectParamDefinition(name='speed', type='float', default=0.42, minimum=0.05, maximum=3.0, unit='multiplier'),
            'min_brightness': EffectParamDefinition(name='min_brightness', type='float', default=0.2, minimum=0.0, maximum=1.0, unit='ratio'),
        },
        defaults={
            'color': '#FFD84D',
            'secondary_color': '#7A5A00',
            'brightness': 0.85,
            'speed': 0.42,
            'min_brightness': 0.2,
        },
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        level = _breath(_elapsed(ctx), _float(params.get('speed'), 0.42), _clamp01(_float(params.get('min_brightness'), 0.20)))
        first = _scaled(params, 'color', 0xFFD84D, level)
        second = _scaled(params, 'secondary_color', 0x7A5A00, level)
