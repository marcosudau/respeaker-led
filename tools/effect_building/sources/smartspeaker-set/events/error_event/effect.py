...
class ErrorEvent(BaseEffect):
    definition = EffectDefinition(
        id='error_event',
        title='Fehler',
        description='Zwei bis drei schnelle rote Pulse; kritische Presets können eine zweite Pulsgruppe wiederholen.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#FF3038'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=900, minimum=200, maximum=10000, unit='ms'),
            'pulse_count': EffectParamDefinition(name='pulse_count', type='int', default=3, minimum=1, maximum=6),
            'repeat_count': EffectParamDefinition(name='repeat_count', type='int', default=1, minimum=1, maximum=3),
        },
        defaults={
            'color': '#FF3038',
            'brightness': 1.0,
        ...
        capabilities=EffectCapabilities(
            playback_modes=(PlaybackMode.SINGLE_RUN,), supports_queueing=True
        ),
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'error'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 900)
        pulses = max(1, _int(params.get('pulse_count'), 3))
        groups = max(1, _int(params.get('repeat_count'), 1))
        scaled = phase * groups
        group_phase = scaled - math.floor(scaled)
        active_window = 0.78
        if group_phase >= active_window:
            intensity = 0.0
        else:
            local = group_phase / active_window
            pulse_phase = (local * pulses) % 1.0
            intensity = math.sin(math.pi * pulse_phase) ** 1.4
        return [_scaled(params, 'color', 0xFF3038, intensity)] * ctx.led_count
