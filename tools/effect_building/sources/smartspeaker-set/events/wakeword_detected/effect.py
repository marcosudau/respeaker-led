...
class WakewordDetectedEvent(BaseEffect):
    definition = EffectDefinition(
        ...
        description='Ein sehr schneller cyanfarbener Vollringblitz bestätigt Wake Word oder erkannte Sprache.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#38D8FF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=300, minimum=80, maximum=2000, unit='ms'),
        },
        defaults={
            'color': '#38D8FF',
            'brightness': 1.0,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'wakeword'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 300)
        if phase < 0.16:
            intensity = phase / 0.16
        else:
            intensity = max(0.0, 1.0 - (phase - 0.16) / 0.84) ** 1.6
        return [_scaled(params, 'color', 0x38D8FF, intensity)] * ctx.led_count
