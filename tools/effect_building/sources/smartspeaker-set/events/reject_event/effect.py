...
class RejectEvent(BaseEffect):
    definition = EffectDefinition(
        id='reject_event',
        title='Ablehnung',
        description='Ein kräftiger roter Gegenschlag fällt rasch ab und endet mit einem kurzen schwächeren Nachimpuls.',
        ...
        parameter_schema={
            ...
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=460, minimum=100, maximum=3000, unit='ms'),
        },
        defaults={
            'color': '#FF3038',
            'brightness': 0.95,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'reject'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 460)
        if phase < 0.52:
            intensity = max(0.0, 1.0 - (phase / 0.52)) ** 0.65
        elif phase < 0.66:
            intensity = 0.0
        else:
            local = (phase - 0.66) / 0.34
            intensity = 0.42 * math.sin(math.pi * local) ** 1.4
