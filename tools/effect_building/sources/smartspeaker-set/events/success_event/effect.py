...
class SuccessEvent(BaseEffect):
    definition = EffectDefinition(
        id='success_event',
        title='Erfolg',
        description='Ein kurzer weicher grüner Vollring-Impuls bestätigt eine erfolgreich abgeschlossene Aktion.',
        ...
        parameter_schema={
            ...
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=520, minimum=100, maximum=5000, unit='ms'),
        },
        defaults={
            'color': '#39D98A',
            'brightness': 0.95,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'success'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 520)
        intensity = math.sin(math.pi * phase) ** 0.8
        return [_scaled(params, 'color', 0x39D98A, intensity)] * ctx.led_count
