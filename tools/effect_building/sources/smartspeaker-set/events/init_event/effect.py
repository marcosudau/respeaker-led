...
class InitEvent(BaseEffect):
    definition = EffectDefinition(
        id='init_event',
        title='Initialisierung / Start',
        description='Der Ring füllt sich vollständig und schließt mit einem kurzen weißen Blitz ab.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#2F80ED'),
            'secondary_color': EffectParamDefinition(name='secondary_color', type='color', default='#FFFFFF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=1400, minimum=200, maximum=10000, unit='ms'),
        },
        defaults={
            'color': '#2F80ED',
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'init'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 1400)
        frame: list[int | None] = [0x000000] * ctx.led_count
        if phase < 0.82:
            exact = (phase / 0.82) * ctx.led_count
            full = min(ctx.led_count, int(math.floor(exact)))
            for index in range(full):
                frame[index] = _scaled(params, 'color', 0x2F80ED)
            if full < ctx.led_count and exact > full:
                frame[full] = _scaled(params, 'color', 0x2F80ED, exact - full)
        else:
            flash_phase = (phase - 0.82) / 0.18
            flash = math.sin(math.pi * flash_phase)
            base = _scaled(params, 'color', 0x2F80ED, 0.55 * (1.0 - flash_phase))
            white = _scaled(params, 'secondary_color', 0xFFFFFF, flash)
            frame = [blend(base, white, flash)] * ctx.led_count
        return frame
