...
class WarnEvent(BaseEffect):
    definition = EffectDefinition(
        id='warn_event',
        title='Warnung',
        description='Langsameres und weicheres gelbes Blinken signalisiert einen unkritischen Hinweis.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#FFD84D'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=1000, minimum=200, maximum=10000, unit='ms'),
            'pulse_count': EffectParamDefinition(name='pulse_count', type='int', default=2, minimum=1, maximum=5),
        },
        defaults={
            'color': '#FFD84D',
            'brightness': 0.88,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'warning'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 1000)
        pulses = max(1, _int(params.get('pulse_count'), 2))
        local = (phase * pulses) % 1.0
        intensity = math.sin(math.pi * local) ** 1.8
        return [_scaled(params, 'color', 0xFFD84D, intensity)] * ctx.led_count
