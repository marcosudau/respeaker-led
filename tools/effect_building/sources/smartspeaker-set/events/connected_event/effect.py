...
class ConnectedEvent(BaseEffect):
    definition = EffectDefinition(
        id='connected_event',
        title='Verbunden',
        description='Ein kurzer grüner Rundlauf mit sanftem Ausblenden signalisiert eine erfolgreiche Verbindung.',
        ...
        parameter_schema={
            ...
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=750, minimum=150, maximum=5000, unit='ms'),
            'reverse': EffectParamDefinition(name='reverse', type='bool', default=False),
            'trail_length': EffectParamDefinition(name='trail_length', type='int', default=4, minimum=1, maximum=12),
        },
        defaults={
            'color': '#39D98A',
            'brightness': 0.95,
        ...
        animated=False,
        directional=True,
        tags=('smartspeaker-set', 'event', 'connected'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if not ctx.led_count:
            return frame
        phase = _progress(ctx, params, 750)
        direction = _direction(params)
        head = min(ctx.led_count - 1, int(math.floor(phase * ctx.led_count)))
        head = head if direction > 0 else ctx.led_count - 1 - head
        fade = 1.0 if phase < 0.72 else max(0.0, (1.0 - phase) / 0.28)
        length = max(1, min(ctx.led_count, _int(params.get('trail_length'), 4)))
        for offset in range(length):
            factor = fade * (1.0 - offset / max(1, length))
            if factor > 0.001:
                frame[(head - offset * direction) % ctx.led_count] = _scaled(params, 'color', 0x39D98A, factor)
        return frame
