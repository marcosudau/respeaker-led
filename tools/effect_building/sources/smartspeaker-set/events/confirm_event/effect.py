...
class ConfirmEvent(BaseEffect):
    definition = EffectDefinition(
        id='confirm_event',
        title='Bestätigung',
        description='Ein kurzer, besonders weicher grüner Puls bestätigt die Annahme einer Eingabe.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#48D597'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=380, minimum=100, maximum=3000, unit='ms'),
        },
        defaults={
            'color': '#48D597',
            'brightness': 0.88,
        ...
        animated=False,
        directional=False,
        tags=('smartspeaker-set', 'event', 'confirm'),
    )

    def render(self, ctx: RenderContext) -> list[int | None]:
        params = _params(ctx)
        phase = _progress(ctx, params, 380)
        intensity = math.sin(math.pi * phase) ** 1.25
        return [_scaled(params, 'color', 0x48D597, intensity)] * ctx.led_count
