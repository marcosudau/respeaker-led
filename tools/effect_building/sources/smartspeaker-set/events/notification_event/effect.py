...
class NotificationEvent(BaseEffect):
    definition = EffectDefinition(
        ...
        description='Ein kurzer freundlicher Sweep mit weichem Schweif weist auf eine neue Nachricht oder einen Hinweis hin.',
        definition_type=DefinitionType.EVENT,
        parameter_schema={
            'color': EffectParamDefinition(name='color', type='color', default='#39D7FF'),
            'brightness': EffectParamDefinition(name='brightness', type='float', default=1.0, minimum=0.0, maximum=1.0, unit='ratio'),
            'duration_ms': EffectParamDefinition(name='duration_ms', type='duration_ms', default=850, minimum=150, maximum=5000, unit='ms'),
            'reverse': EffectParamDefinition(name='reverse', type='bool', default=False),
            'trail_length': EffectParamDefinition(name='trail_length', type='int', default=5, minimum=1, maximum=12),
        },
        defaults={
            'color': '#39D7FF',
            'brightness': 0.9,
        ...
        capabilities=EffectCapabilities(
            playback_modes=(PlaybackMode.SINGLE_RUN,), supports_transparency=True, supports_queueing=True
        ),
        ...
        animated=False,
        directional=True,
        tags=('smartspeaker-set', 'event', 'notification'),
    )
    ...
    def render(self, ctx: RenderContext) -> list[int | None]:
        ...
        if not ctx.led_count:
            return frame
        phase = _progress(ctx, params, 850)
        direction = _direction(params)
        head = int(math.floor(phase * (ctx.led_count + max(1, _int(params.get('trail_length'), 5)))))
        length = max(1, min(ctx.led_count, _int(params.get('trail_length'), 5)))
        for offset in range(length):
            logical = head - offset
            if 0 <= logical < ctx.led_count:
                index = logical if direction > 0 else ctx.led_count - 1 - logical
                factor = (1.0 - offset / max(1, length)) * math.sin(math.pi * min(1.0, phase + 0.18))
                frame[index] = _scaled(params, 'color', 0x39D7FF, factor)
        return frame
