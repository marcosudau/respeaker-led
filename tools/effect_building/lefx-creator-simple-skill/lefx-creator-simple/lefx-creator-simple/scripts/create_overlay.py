...
EFFECT_YAML = dedent(
    """
    # Diese Datei wurde von scripts/create_overlay.py erzeugt.
...
PRESETS_YAML = dedent(
    """
    # Presets konfigurieren nur die stabile Effektkonfiguration.
    #
    # REQUIRED:
    # - Alle Platzhalter ersetzen.
    # - Nur Felder aus parameter_schema eintragen.
    # - Keine Werte aus runtime_input_schema in Presets eintragen.
    #
...
EFFECT_PY = dedent(
    ...
    class <CLASS_NAME>(BaseEffect):
        definition = EffectDefinition(
            ...
            # Durch create_overlay.py bereits festgelegt.
            definition_type=DefinitionType.OVERLAY,

            # REQUIRED - Ergebnis aus Abschnitt 2B eintragen.
            ...
            # REQUIRED:
            # - Timed Overlay: leeres Dictionary {}.
            # - Controlled Overlay: nur die waehrend der Nutzung
            #   aktualisierbaren Werte deklarieren; {} ist ebenfalls erlaubt.
            # Siehe Abschnitt 2B, Frage 2.
            ...
            # REQUIRED - Ergebnis aus Abschnitt 2B und Abschnitt 6 eintragen.
            # Den vollstaendigen capabilities/layer_rules-Block aus Abschnitt 6.2
            # an dieser Stelle einsetzen.
            <OVERLAY_CAPABILITIES_AND_LAYER_RULES>
        ...
        def render(self, ctx: RenderContext) -> list[int | None]:
            ...
            # - bei transparenter Komposition fuer unbeteiligte LEDs None nutzen.
            # - zeitliche Bewegung aus Enginezeit berechnen.
            # - nur deklarierte und validierte params/inputs verwenden.
            ...
            del params, inputs
            raise NotImplementedError("<IMPLEMENT_OVERLAY_RENDER_LOGIC>")
    '''
).lstrip()


def request_target_directory() -> Path:
    raw = input("Zielordner fuer die neue Overlay-Effektquelle: ").strip()
    if not raw:
...
def main() -> None:
    ...
    print(f"Overlay-Scaffold erstellt: {target}")
    print("Erzeugt: effect.yaml, effect.py, presets.yaml")
    print("Naechster Schritt: SKILL.md ab Abschnitt 2B durcharbeiten.")
