...
# DoA - Konzept, Integration und Template

Hinweis: Das hier beschriebene Projekt wurde tatsächlich nie in das Projekt integriert. Es wurde nach einigen experimentellen Versuchen erstmal  auf Eis gelegt, da die DoA-Visualisierung nicht als kritisch für den Betrieb angesehen wurde. Die Idee und das Konzept ist aber weiterhin interessant und kann bei Bedarf wieder aufgegriffen werden.
...
- `src/python_control/xvf_host.py` kennt bereits `DOA_VALUE` und beschreibt dabei das Payload-Schema als:
  - `payload[0] = doa value`
  - `payload[1] = 1 if speech is detected, 0 if not`
...
- `DOA_VALUE` ueber das bereits genutzte `xvf_host.py` lesen
